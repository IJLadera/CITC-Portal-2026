...
def user_energy_cost(request):
    ...
    # Clear week when month is selected for month-only filtering
    if selected_month and not selected_day:
        selected_week = None
    ...
    if not any([selected_day, selected_month, selected_year, selected_week]):
        ...
        if latest_data['latest_date']:
            ...
            if week_options and not selected_month:
    ...
    if level == 'day':
        ...
        # Find highest cost week for month view
        year = int(selected_year) if selected_year else dt.now().year
        month = int(selected_month) if selected_month else dt.now().month
        ...
        while current_week_start <= end_date:
            current_week_end = min(current_week_start + timedelta(days=6), end_date)
            week_energy = SensorReading.objects.filter(
                device__in=devices,
                date__gte=current_week_start,
                date__lte=current_week_end
            ).aggregate(total=Sum('total_energy_kwh'))['total'] or 0
            
            if week_energy > highest_week_energy:
                highest_week_energy = week_energy
                highest_week_num = week_num
            
            current_week_start += timedelta(days=7)
            week_num += 1
        
        highest_cost_day = f"Week {highest_week_num}"
    else:
    ...
    if level == 'week':
        ...
        while current_week_start <= end_date:
            ...
            chart_labels.append([f"Week {week_num}", f"{current_week_start.strftime('%m/%d')} - {current_week_end.strftime('%m/%d')}"])
            chart_data.append(week_energy)
        ...
        if chart_data and any(v > 0 for v in chart_data):
            max_energy = max(chart_data)
            max_index = chart_data.index(max_energy)
            month_names = ['January', 'February', 'March', 'April', 'May', 'June',
                          'July', 'August', 'September', 'October', 'November', 'December']
            highest_cost_day = month_names[max_index]
        else:
            highest_cost_day = 'N/A'
    else:  # day
...
def user_emmision(request):
    ...
    # Clear week when month is selected for CO2 emission filtering
    if selected_month and not selected_day:
        selected_week = None
    ...
    if level == 'week':
        ...
        while current_week_start <= end_date:
            ...
            week_co2 = week_energy * co2_settings.co2_emission_factor
            labels.append(f"Week {week_num}")
            week_data.append(week_co2)
