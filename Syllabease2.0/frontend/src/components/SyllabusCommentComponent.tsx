...
interface SyllabusCommentsProps {
  syllabusId: number;
  section: string;
  direction?: "right" | "left" | "top" | "bottom";
}

export default function SyllabusCommentComponent({
  syllabusId,
  section,
  direction = "right",
}: SyllabusCommentsProps) {
  ...
  const fetchComments = async () => {
    try {
      const res = await api.get(
        `/syllabus-comments/by_section/?syllabus_id=${syllabusId}&section=${section}`
      );
  ...
  const handleAddComment = async () => {
    ...
    try {
      await api.post("/syllabus-comments/", {
        syllabus: syllabusId,
        section,
  ...
  const handleResolve = async (id: number) => {
    try {
      ...
      await api.patch(`/syllabus-comments/${id}/`, { is_resolved: true, resolved_at: now  });
  ...
  const handleEdit = async (id: number, updatedText: string) => {
    try {
      await api.patch(`/syllabus-comments/${id}/`, { text: updatedText });
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      await api.delete(`/syllabus-comments/${id}/`);
