...
export default function CurriculaPage() {
  ...
  // Fetch curricula and programs together and show loading overlay
  useEffect(() => {
    let mounted = true;
    const load = async () => {
      try {
  ...
  const validateForm = () => {
    ...
    if (!formData.program_id) errors.program_id = "Department Program is required.";
  ...
  return (
    <div className="flex-1 flex flex-col p-4">
      ...
      <Modal dismissible show={openModal} onClose={() => setOpenModal(false)}>
        <ModalHeader>{editMode ? "Edit Curriculum" : "Create Curriculum"}</ModalHeader>
        <ModalBody>
          <div className="space-y-4">
            <div>
              ...
              <TextInput
                ...
                onChange={(e) => setFormData({ ...formData, curr_code: e.target.value })}
                required
              />
              ...
              <table className="w-full">
                <thead>
                  <tr className="bg-[#007BFF] text-white text-sm w-full border border-radius-sm">
                    <th className="p-2">Curriculum Code</th>
                    <th className="p-2">Effectivity</th>
