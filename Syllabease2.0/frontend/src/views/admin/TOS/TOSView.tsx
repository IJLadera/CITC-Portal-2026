...
import { FaEdit, FaChevronRight, FaChevronLeft, FaListAlt, FaCheckCircle } from "react-icons/fa";
import { Undo2, CheckCircle } from "lucide-react"; 
import { Modal, ModalHeader, ModalBody, ModalFooter, Button, Spinner } from "flowbite-react"; // assuming you use flowbite-react 
...
export default function TOSView() {   
  ...
  const handleSubmitTOS = async () => {  
    ...
    const leader = tos.bayanihan_group?.bayanihan_members?.find(
      (member: any) => member.role === "LEADER"
    )?.user;

    if (!leader?.signature) {
      toast.error("Cannot submit syllabus because the Bayanihan Leader does not have a signature set. Please upload the signature in the leader's profile first.");
      return;
    }
  ...
  const handleReview = async (decision: "approve" | "reject") => {
    ...
    try {
      setIsClicking(true);
      if (decision === "approve") {
        const res = await api.patch(`/tos/${tos.id}/review-tos/?role=${role}`,
          { decision: "approve" }
        );   
        setTos(res.data);  
        ...
        const res = await api.patch(
          `/tos/${tos.id}/review-tos/?role=${role}`,
          { decision: "reject" }
        );  
        setTos(res.data); 
          ...
          case "Requires Revision":
            return (
              <div className="flex flex-row px-18 justify-between items-center font-normal">
                ...
                <div>
                  <div className={`bg-green-300 py-2 px-3 text-green-600 rounded shadow-lg hover:scale-105 transition ease-in-out ${submitLoading ? "opacity-70 cursor-not-allowed" : "hover:scale-105 cursor-pointer"}`}>
                    <button
