...
export default function SyllabusView() { 
  ...
  // Loading overlay state (shows spinner while fetching/updating syllabus)
  const [loading, setLoading] = useState(true);

  // Submit-button full-page loading overlay
  ...
  const handleSubmitSyllabus = async () => { 
    ...
    if (!user?.signature) {
        return fail(`You cannot submit the syllabus because your account does not have a signature set yet. Please upload your signature in your profile before proceeding.`);
    }
  ...
  // keep a fallback if there's no syllabus and loading finished
  ...
  return (
    <div className="font-thin min-h-screen my-14">  
      ...
      <button
        type="button"
        onClick={() => navigate("/bayanihan_leader/syllabus/")}
          ...
          case "Requires Revision":
            return (
              <div className="flex flex-row justify-between items-center ml-16 font-normal">
                ...
                <div>
                  <div className={`bg-green-300 py-2 px-3 mr-16 text-green-600 rounded shadow-lg transition ease-in-out 
                    ${submitLoading ? "opacity-70 cursor-not-allowed" : "hover:scale-105 cursor-pointer"}`}
                  >
                    <button
                      type="button"
                    ...
                    >
                      ...
                      <span>{submitLoading ? "Submitting Syllabus" : "Submit Syllabus"}</span>
                    </button>
                  </div>
                </div> 
      ...
      `}>
        ...
        {/* SYLLABUS TABLE */}
        <table className="mt-2 mx-auto border-2 border-solid border-black w-full max-w-full text-sm bg-white font-[Times-New-Roman]">
          <tbody>
            {/* 1st Header */}
            <tr>
            ...
            <tr>
              ...
              <td colSpan={2} className="w-[80%] align-top border-2 border-solid border-black">
                <table className="my-4 mx-2">
                  <tbody>
                    <tr>
                      <td className={`border-2 border-solid font-medium text-left px-4 max-w-[250px] wrap-break-word whitespace-pre-line w-1/2 relative 
