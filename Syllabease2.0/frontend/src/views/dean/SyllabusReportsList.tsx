...
function getDeadlineStatus(report: Report, deadlines: Deadline[]): "on-time" | "near" | "overdue" {
  const collegeId = report.syllabus.college.id;
  ...
  const deadlineDate = new Date(matched.syll_deadline);
  const now = new Date();
...
export default function SyllabusReportsList() {
  ...
  useEffect(() => {
    ...
    loadData();
  }, [currentPage, searchTerm, filterYear, filterSemester, filterProgram, filterStatus, showApprovedOnly, role]);  
  ...
  const openVersionsModal = async (report: Report) => {
    ...
    try {
      const res = await api.get<VersionsResponse>(`/reports/${report.id}/`);
          ...
          <div className="max-w-7xl mx-auto space-y-4">
            ...
            >
              <h1 className="text-4xl font-bold text-gray-200 tracking-tight">
                Syllabus Reports Dashboard
              </h1>
              <p className="text-gray-200 mt-2 font-semibold">
                Track Syllabus Submissions
              </p>
            </div>
            ...
            <Card className="bg-blue-600 text-white shadow-lg border-none hover:shadow-xl transition-all">
              <CardHeader>
                <CardTitle className="text-lg font-semibold tracking-wide">
                  Total Active Syllabus Reports
                </CardTitle>
              </CardHeader>
              <CardContent>
                <p className="text-blue-100 mb-3">
                  View and filter all Syllabus submissions and approval records.
                </p>
                  ...
                  <Table>
                    <TableHeader>
                      <TableRow className="bg-blue-100 text-blue-900 text-center">
                        ...
                        <TableHead className="text-center">Approved by Dean</TableHead>
                    ...
                    <TableBody>
                      {reports.map((r) => {
                        ...
                        const matchedDeadline = deadlines.find(
                          (d) =>
                            d.school_year === r.bayanihan_group.school_year &&
                            d.semester.toLowerCase() ===
                              r.bayanihan_group.course.course_semester.toLowerCase()
                        ); 

                        const deadlineDate = matchedDeadline
                          ? formatDate(matchedDeadline.syll_deadline)
                          : "—";
                        ...
                        return (
                          <TableRow key={r.id} className={`transition-all ${rowClass}`}>
                            ...
                            <TableCell>{r.bayanihan_group.bayanihan_members
                            ...
                            <TableCell>
                              <Badge
                                style={statusStyles[r.syllabus.status] || statusStyles["Draft"]}
                ...
                <Table className="w-full text-left">
                  <TableHeader>
                    <TableRow className="text-sm text-slate-600"> 
                      ...
                      <TableHead>Approved by Dean</TableHead>
                  ...
                  <TableBody>
                    {selectedReportVersions.map((v) => { 
                      ...
                      const matchedDeadline = deadlines.find(
                        (d) =>
                          d.school_year === v.bayanihan_group.school_year &&
                          d.semester.toLowerCase() ===
                            v.bayanihan_group.course.course_semester.toLowerCase()
                      ); 
                      ...
                      return ( 
                        <TableRow key={v.id} className={`transition-all ${rowClass}`}> 
                          ...
                          <TableCell>
                            <Badge
                              style={
                                statusStyles[v.syllabus.status] || statusStyles["Draft"]
                              }
