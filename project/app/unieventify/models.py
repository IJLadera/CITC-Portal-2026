...
class tblUserRole(models.Model):
    ...
    rank = models.IntegerField(null=True)  # Lower number means higher rank (e.g., 1 = Admin, 2 = Dean, etc.)
...
class tblcollege(models.Model):
    ...
    def __str__(self):
        try:
            return f'{self.collegeName}'
        except Exception as e:
            return f'Invalid Data ({self.pk}): {e}'
    
class tbldepartment(models.Model):
    collegeName = models.ForeignKey(tblcollege, related_name='departments', on_delete=models.CASCADE, null=True)
...
class tblYearLevel(models.Model):
...
class tblSection(models.Model):
    tblYearLevel = models.ForeignKey(tblYearLevel, on_delete=models.CASCADE, null=True, blank=True)
...
#     # Generate the filename using the user's last name and original file extension
#     filename = f"{instance.last_name}.{ext}"
#     # Return the full upload path
#     return os.path.join('profiles', instance.last_name, filename)
...
#     role = models.ForeignKey(tblUserRole, on_delete=models.CASCADE, null=True, blank=True)
#     department = models.ForeignKey(tbldepartment, on_delete=models.CASCADE, null=True, blank=True)
#     section = models.ForeignKey(tblSection,on_delete=models.CASCADE, null=True, blank=True)
#     organization = models.ForeignKey(tblstudentOrg,on_delete=models.CASCADE, null=True, blank=True)
...
class tblEvent(models.Model):
    ...
    department = models.ManyToManyField(tbldepartment, blank=True)
    meetinglink = models.TextField(null=True, blank=True)
    participants = models.ManyToManyField('users.User', related_name='event_participants', blank=True)
...
class tblSchoolYear(models.Model): 
    schoolYearName = models.CharField(max_length=255)
    startYear = models.IntegerField()
    endYear = models.IntegerField()

    def __str__(self):
        try:
            return f'{self.schoolYearName}'
        except Exception as e:
            return f'Invalid Data ({self.pk}): {e}'
...
class tblSemester(models.Model): 
    ...
    schoolYear = models.ForeignKey(tblSchoolYear, on_delete=models.CASCADE, null=True, blank=True)
...
# Register Model in the audit log
# auditlog.unregister(tblEventRemarks)
# auditlog.unregister(tblEventSchoolYearAndSemester)
# auditlog.unregister(tblSemester)
# auditlog.unregister(tblSchoolYear)
# auditlog.unregister(tblEventLog)
auditlog.register(tblEvent)
# auditlog.unregister(tblStatus)
# auditlog.unregister(tblSetup)
# auditlog.unregister(tblVenue)
# auditlog.unregister(tblEventCategory)
# auditlog.unregister(tblEventType)
# auditlog.unregister(CustomUser)
# auditlog.unregister(tblSection)
# auditlog.unregister(tblYearLevel)
# auditlog.unregister(tblstudentOrg)
# auditlog.unregister(tbldepartment)
# auditlog.unregister(tblcollege)
# auditlog.unregister(tblUserRole)
