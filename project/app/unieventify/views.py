...
from .serializers import (
    tblUserRoleSerializer, tblcollegeSerializer, 
    tbldepartmentSerializer, tblstudentOrgSerializer, tblSectionSerializer, tblEventTypeSerializer,
    ...
    CollegeSerializer, DesignationCountSerializer, 
    UserEventSerializer,FacultyEventSerializer, AllRoleEventSerializer, 
    tblYearLevelSerializer, SchoolYearAndSemesterEvent, SchoolYearSerializer, SemesterSerializer, tblEventType,
    EventRemarkSerializer, CSVUploadSerializer, ApproveDocumentsSerializer, LogEntrySerializer
    )
from users.serializers import UpdateUserSerializer, ChangePasswordSerializer, CustomUserSerializer
...
from users.permissions import IsOwnerOrReadOnly, IsAdminOrReadOnly, IsUserOrReadOnly, IsDeanOrReadOnly, IsUserOrIsAdminOrReadOnly, IsNotificationOrReadOnly, IsOwnerOrIsAdminOrReadOnly, RoleHierarchyPermission, IsAuthenticatedOrReadOnly
...
class UserRoleListView(ListCreateAPIView):
    queryset = tblUserRole.objects.all()
    serializer_class = tblUserRoleSerializer
    permission_classes = [IsAdminOrReadOnly]
...
class YearLevelListView(ListCreateAPIView):
    queryset = tblYearLevel.objects.all()
    serializer_class = tblYearLevelSerializer
    permission_classes = [IsAdminOrReadOnly]
...
class SectionListView(ListCreateAPIView):
    queryset = tblSection.objects.all()
    serializer_class = tblSectionSerializer
    permission_classes = [IsAdminOrReadOnly]
...
class SchoolYearListView(ListCreateAPIView):
    queryset = tblSchoolYear.objects.all()
...
class EventListView(ListCreateAPIView):
    ...
    def perform_create(self, serializer):
        ...
        if event.startDateTime:
            ...
            school_years = tblSchoolYear.objects.filter(
...
class EventStatisticsView(APIView):
    ...
    def get_queryset(self):
        ...
        if user.role.designation == 'Admin':
            return queryset
        elif user.role.designation == 'Dean':
            return queryset.exclude(created_by__role__designation__in=['Mother Org', 'Unit Org'])
        elif user.role.designation in ['Chairperson', 'Faculty']:
            return queryset.filter(
                Q(department=user.department) | 
                Q(participants__in=[user])
            ).exclude(created_by__role__designation__in=['Mother Org', 'Unit Org'])
        elif user.role.designation == 'Student':
            return queryset.filter(
                Q(department=user.department) & 
                Q(participants__in=[user])
            )
        elif user.role.designation == 'Mother Org':
            return queryset.filter(
                created_by__role__designation__in=['Mother Org', 'Unit Org']
            )
        elif user.role.designation == 'Unit Org':
            return queryset.filter(
                (Q(department=user.department) & Q(created_by__role__designation__in=['Mother Org', 'Unit Org'])) |
                Q(created_by__role__designation='Mother Org')
            )
                    ...
                    'events': [
                        {
                            ...
                            'created_by': f"{e.created_by.first_name} {e.created_by.last_name} ({e.created_by.role.designation})",
                            'participants': [f"{p.first_name} {p.last_name}" for p in e.participants.all()],
                            'departments': [d.departmentName for d in e.department.all()],
...
class EventStatisticsByDepartmentView(APIView):
    ...
    def get_queryset(self):
        ...
        queryset = tblEvent.objects.exclude(eventCategory__eventCategoryName__iexact='personal') \
                                   ...
                                   .exclude(department__departmentName='') \
    ...
    def get(self, request):
        ...
        events_by_department = events_queryset.values('department__departmentName').annotate(
        ...
        for department in events_by_department:
            department_name = department['department__departmentName']
            department_events = events_queryset.filter(department__departmentName=department_name)
        ...
        for row in reader:
           ...
           # Handle department field with custom logic
            ...
            if department_names == "All":
                ...
                departments = tbldepartment.objects.exclude(departmentName__startswith="All")
            elif department_names == "CITC":
                # Dynamically fetch the college based on name "CITC"
                citc_college = tblcollege.objects.filter(collegeName="CITC").first()
                if citc_college:
                    # Select departments under the CITC college but exclude those that start with "All"
                    departments = tbldepartment.objects.filter(
                        Q(collegeName=citc_college) & ~Q(departmentName__startswith="All")
                    )
            else:
                # Split and match specific department names
                departments = tbldepartment.objects.filter(
                    departmentName__in=[name.strip() for name in department_names.split(',')]
                )
            ...
            data = {
                ...
                'participants': [participant.id for participant in participants] if participants else [],
                'eventName': eventName,
        ...
        if serializer.is_valid():
            ...
            for row in csv_data:
                try:
                    ...
                    required_fields = ["email", "first_name", "last_name", "idNumber"]
                    ...
                    role = tblUserRole.objects.filter(designation__iexact=row.get('role', '').strip()).first()
                    department = tbldepartment.objects.filter(departmentName__iexact=row.get('department', '').strip()).first()
                    section = tblSection.objects.filter(sectionName__iexact=row.get('section', '').strip()).first()
                    ...
                    user = User.objects.filter(idNumber=row['idNumber']).first()
                    ...
                    if user:
                        ...
                        user.set_password(user.idNumber)
                        user.save()
        ...
        if user.role.designation=="Dean":
            ...
            return (
                tblEvent.objects.filter(
                    ...
                    Q(created_by__role__designation__in=["Mother Org", "Unit Org"]),
                    Q(status__statusName="draft"),
                )
            )
        
        # Check if the user is a Chairperson with an assigned department
        if user.role.designation == "Chairperson" and user.department.id:
            # Retrieve event logs for Unit Org or Faculty within the same department
            return (
                tblEvent.objects.filter(
                    ~Q(eventCategory__eventCategoryName="Personal"),
                    Q(created_by__role__designation__in=["Unit Org", "Faculty"]),
...
class DesignationCountView(APIView):
    def get(self, request):
        # Aggregate counts of each designation from tblUserRole
        designation_counts = tblUserRole.objects.annotate(
            count=Count('user')
        ).values('designation', 'count')

        # Add users list for each designation
        designation_data = []
        for item in designation_counts:
            # Fetch users related to each designation using the foreign key (role)
            users = User.objects.filter(role__designation=item['designation']).values(
                'id', 'first_name', 'last_name'
            )
            item['users'] = users
            designation_data.append(item)
    ...
    def create_time_slots(self, events):
        ...
        for event in events:
            ...
            participants = [{
                ...
                'designation': p.role.designation if p.role else None,
                'rank': p.role.rank if p.role else None,
                'department': p.department.departmentName if p.department else None,
            } for p in event.participants.all()]

            created_by = {
                'email': event.created_by.email,
                'idNumber': event.created_by.idNumber,
                'first_name': event.created_by.first_name,
                'last_name': event.created_by.last_name,
                'designation': event.created_by.role.designation if event.created_by.role else None,
                'rank': event.created_by.role.rank if event.created_by.role else None,
                'department': event.created_by.department.departmentName if event.created_by.department else None
            }

            departments = [{"id": dept.id, "departmentName": dept.departmentName} for dept in event.department.all()]
...
class FacultyEventsListView(ListAPIView):
    ...
    def get_queryset(self):
        ...
        return User.objects.filter(role__designation='Faculty').prefetch_related(
...
class AllRoleEventsListView(ListAPIView):
    ...
    def get_queryset(self):
        return User.objects.filter(is_staff=False).prefetch_related(
            'event_participants',  # Events they participate in
            'event_createdby'      # Events they created
            ).order_by('role__rank')  # Order by role rank
...
class FilteredEventsByCurrentSemesterView(ListAPIView):
    ...
    def get_queryset(self):
        ...
        elif user.role.designation in ["Chairperson", "Faculty", "Student"]:
